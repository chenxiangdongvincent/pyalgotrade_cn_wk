...
from pyalgotrade.wukongTailored import broker
from pyalgotrade.wukongTailored.broker import fillstrategy
from pyalgotrade import logger
import pyalgotrade.wukongTailored.windFeed.bar

#引入pandas包，用来存储每日持仓与交易流水
import pandas as pd
import numpy as np
...
class Broker(broker.Broker):
    ...
    #返回dataframe
    def getPdShares(self):
        return self.__pdShares
    
    def getPdTradeShares(self):
        return self.__pdTradeTracker
    ...
    def commitOrderExecution(self, order, dateTime, fillInfo):
        ...
        if resultingCash >= 0 or self.__allowNegativeCash:
            ...
            #记录交易流水,此处需要修改
            self.__tradeTracker[order.getInstrument()] = sharesDelta
    ...
    def submitOrder(self, order):
        if order.isInitial():
            ...
            #print "in submitOrder, change state to submitted and call notifyOrderEvent"
    ...
    def __onBarsImpl(self, order, bars):
        ...
        if bar_ is not None:
            ...
            if bar_.getUpDownStatus() == 1 and order.isBuy():
                print "price up to 10%, can not buy"
                
                if not order.isCanceled():
                    order.switchState(broker.Order.State.CANCELED)
            elif bar_.getUpDownStatus() == -1 and order.isSell():
                print "price down to -10%, can not sell"
                if not order.isCanceled():
                    order.switchState(broker.Order.State.CANCELED)
